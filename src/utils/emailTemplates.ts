...
const emailStyles = {
  container:
    'font-family: Jost, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; max-width: 600px; margin: 0 auto; background-color: #ffffff;',
  header:
    'padding: 40px 30px; text-align: center; background-color: #f7fafc;',
  headerTitle:
    'color: #1f2937; font-size: 32px; font-weight: 400; margin: 0; letter-spacing: 0.2em; text-transform: uppercase;',
  headerSubtitle: 'color: #6b7280; font-size: 18px; font-weight: 400; margin: 15px 0 0 0; letter-spacing: 0.1em; text-transform: uppercase;',
  content: 'padding: 40px 30px; line-height: 1.6;',
  qaItem: 'margin-bottom: 30px;',
  question: 'color: #2d3748; font-size: 18px; font-weight: normal; margin: 0 0 15px 0;',
  answer: 'padding: 15px; border-left: 2px solid #428bb0; background-color: #f8fafc;',
  text: 'color: #4a5568; font-size: 16px; margin: 0; line-height: 1.6;',
  highlight: 'color: #428bb0; font-weight: 600;',
  button:
    'display: inline-block; background-color: #428bb0; color: #ffffff; padding: 12px 30px; text-decoration: none; border-radius: 6px; font-weight: 600; margin: 20px 0;',
};
...
export function generateGuestConfirmationEmail(data: RSVPData): { subject: string; html: string } {
  ...
  const attendanceMessage = isAttending
    ? `<div style="${emailStyles.qaItem}">
         <h3 style="${emailStyles.question}">We're so excited you'll be joining us!</h3>
       </div>`
    : `<div style="${emailStyles.qaItem}">
         <h3 style="${emailStyles.question}">Thank you for letting us know</h3>
         <div style="${emailStyles.answer}">
  ...
  const eventDetails = isAttending
    ? `
    <div style="${emailStyles.qaItem}">
      <h3 style="${emailStyles.question}">Event Details</h3>
      <div style="${emailStyles.answer}">
        <p style="${emailStyles.text}">
  ...
  const rsvpDetails = `
    <div style="${emailStyles.qaItem}">
      <h3 style="${emailStyles.question}">Your RSVP Details</h3>
      <div style="${emailStyles.answer}">
        <p style="${emailStyles.text}">
  ...
  const changesNote = isAttending
    ? `
    <div style="${emailStyles.qaItem}">
      <h3 style="${emailStyles.question}">Need to Make Changes?</h3>
      <div style="${emailStyles.answer}">
        <p style="${emailStyles.text}">
  ...
  const html = `
    ...
    <body style="margin: 0; padding: 20px; background-color: #f7fafc;">
      <div style="${emailStyles.container}">
        ...
        <div style="${emailStyles.content}">
          <div style="${emailStyles.qaItem}">
            <h3 style="${emailStyles.question}">Guest Information</h3>
            <div style="${emailStyles.answer}">
              <p style="${emailStyles.text}">
          ...
          ${
            data.plusOne
              ? `
            <div style="${emailStyles.qaItem}">
              <h3 style="${emailStyles.question}">Plus One Information</h3>
              <div style="${emailStyles.answer}">
                <p style="${emailStyles.text}">
